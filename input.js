...
window.addEventListener("click", e => {
  ...
  if (Math.abs(x) > Math.abs(y)) {
    if (x > 0 && lastInputDirection.x === 0) inputDirection = { x: 1, y: 0 }
    else if (lastInputDirection.x === 0) inputDirection = { x: -1, y: 0 }
  }
  else {
    if (y > 0 && lastInputDirection.y === 0) inputDirection = { x: 0, y: 1 }
    else if (lastInputDirection.y === 0) inputDirection = { x: 0, y: -1 }
  }
})
